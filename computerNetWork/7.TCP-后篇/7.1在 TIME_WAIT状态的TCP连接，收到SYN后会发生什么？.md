## 在 TIME_WAIT状态的TCP连接，收到SYN后会发生什么？

《Linux 服务器高性能编程》上有这么一句话：
...
书中作者只是提了这么一句话，没有给出源码或者抓包图的证据。

这个逻辑看似符合常理的，但是当我自己去啃了 TCP 源码后，发现并`不是这样的`。

所以，「**在 TCP 正常挥手过程中，处于 TIME_WAIT 状态的连接，收到相同四元组的 SYN 后会发生什么？**」

![图片](..\imgs\处于TIME_WAIT状态又收到相同四元组的SYN.png)

## 1.先说结论

这个问题，**关键要看 SYN 的「序列号和时间戳」是否合法**，因为处于 TIME_WAIT 状态的连接收到 SYN 后，会判断 SYN 的「序列号和时间戳」是否合法，然后根据判断结果做不同的处理。
...
上面 SYN 合法判断是基于双方都开启了 TCP 时间戳机制的场景，如果双方都没有开启，则 SYN 合法判断如下：
...
如果处于 TIME_WAIT 状态的连接收到「合法 」的 SYN，**就会重用此四元组连接，跳过 2MSL 而转变为 SYN_RECV 状态，接着就能进行建立连接过程**。

如下图，双方都启用了 TCP 时间戳机制，TSval 是发送报文时的时间戳：
...
> PS：处于 TIME_WAIT 状态的连接，收到 RST 会断开连接吗？

## 2.源码分析

下面分析是基于 Linux 4.2 版本的内核代码。

Linux 内核在收到 TCP 报文后，会执行 `tcp_v4_rcv` 函数，该函数中和 TIME_WAIT 状态相关的主要代码如下：
...
TIME_WAIT 状态收到 RST 报文而释放连接，等于跳过 2MSL 时间，这么做是有风险的。

sysctl_tcp_rfc1337 这个参数是在 rfc 1337 文档提出来的，目的是避免因为 TIME_WAIT 状态收到 RST 报文而跳过 2MSL 的时间，文档里也给出跳过 2MSL 时间会有什么**潜在问题**。
...
虽然 TIME_WAIT 状态持续的时间是有点长，显得很不友好，但是它被设计来就是用来避免发生乱七八糟的事情。
